...
function parseData(data, elfStrength) {
  const cave = data.split("\n").map((line, y) => {
    return line.split("").map((char, x) => {
      switch (char) {
        ...
        case "E":
          return new Elf(x, y, elfStrength);
      }
    });
  });
  return cave;
...
function countElves(cave) {
  return cave.flat(1).filter(cavern => cavern instanceof Elf).length;
}

function answer(path) {
  const data = fs.readFileSync(path, { encoding: "utf8" });
  outside: for (let elfStrength = 4; true; elfStrength++) {
    const cave = parseData(data, elfStrength);
    const elvesCount = countElves(cave);
    let round = 1;
    ...
    console.log("Strength:", elfStrength);
    break;
  }
}
