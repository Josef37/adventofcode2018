      ...
      .forEach(cart => {
        ...
        if (
          carts
            ...
            .some(
              cart2 =>
                cart !== cart2 && cart.x === cart2.x && cart.y === cart2.y
            )
        ) {
          carts
            .filter(cart2 => cart.x === cart2.x && cart.y === cart2.y)
            .forEach(cart => (cart.isCrashed = true));
        }
      });
    const remainingCarts = carts.filter(cart => !cart.isCrashed);
    if (remainingCarts.length === 1) {
      console.log(remainingCarts[0].x + "," + remainingCarts[0].y);
      break;
    }
  }
}
